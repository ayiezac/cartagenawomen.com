 ...
 * @return {void}
 */
const updateAnchorContent = (anchorSelector, href, content) => {
 ...
 * @return {void}
 */
export const isMobileUserLoggedIn = (badgeTextContent) => {
	if (!ctaSignupModal || !(btnSignInState instanceof HTMLAnchorElement)) return;
	...
	const badgeIconOnMobile = () => {
		return `
            <span class="position-relative">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-mail"><rect width="20" height="16" x="2" y="4" rx="2"></rect><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"></path></svg>
	...
	btnSignInState.setAttribute("noprefetch", "");
 ...
 * @return {void}
 */
export const isMobileUserLoggedOut = () => {
	if (
		!ctaSignupModal ||
		...
		return;
	/**
	 * Returns an SVG element representing a sign up icon with a user-plus symbol and the text "Sign in".
	 *
	 * @return {string} The SVG element as a string.
	 */
	const badgeIconOnMobile = () => {
		return `
            ...
            <small class="mb-0 text-white text-nowrap">Sign in</small>
        `;
	};
	updateAnchorContent(
		'[data-id="isMailUserLoggedIn"]',
	...
	ctaSignupModal.dataset.bsTarget = ".cartagenawomen-modal";
