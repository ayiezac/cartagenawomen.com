...
const worker = new Worker("/imagemap/js/worker-utils/worker-fetchdom.js");
/**
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const data = await workerAPI.fetchData("/members/home/");
    handleRender(data);
});
...
const handleSignupState = () => {
    return `
        <a href="#signup" role="button" class="mb-0 link-light border p-3 d-flex gap-2 rounded-pill align-items-center btn-primary-color" data-bs-toggle="modal" data-bs-target=".cartagenawomen-modal">
...
const handleRender = async (data) => {
    const { isMobileUserLoggedIn, isMobileUserLoggedOut } = await import("/imagemap/js/isUserLoggedInMobile.min.js");
